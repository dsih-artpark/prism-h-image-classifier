...
class SimCLRProjectionHead(nn.Module): # Copied for completeness
    def __init__(self, input_dim, hidden_dim=2048, output_dim=128):
        ...
        self.projection = nn.Sequential(nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, output_dim))
...
class SimCLRModel(nn.Module): # Copied for completeness
     def __init__(self, base_model='resnet50', pretrained=True, output_dim=128):
         ...
         else: raise ValueError(f"Unsupported base model: {base_model}")
         self.encoder.fc = nn.Identity()
         self.projection_head = SimCLRProjectionHead(input_dim=self.encoder_dim, output_dim=output_dim)
     def forward(self, x):
         features = self.encoder(x); projections = self.projection_head(features); return features, projections
...
class Classifier(nn.Module):
    def __init__(self, simclr_checkpoint_path, num_classes, freeze_encoder=True, base_model='resnet50'):
        ...
        # Load the SimCLR model structure
        # Set pretrained=False as we'll load weights from checkpoint
...
def train_classifier(model, train_loader, val_loader, criterion, optimizer, device, epochs=50, checkpoint_dir="classification_checkpoints", patience=5):
    ...
    for epoch in range(epochs):
        ...
        if not train_targets:
            print(f"Warning: No valid training samples found in Epoch {epoch+1}. Skipping epoch metrics.")
            continue

        epoch_loss = running_loss / len(train_targets) # Use len(train_targets) as it reflects actual processed samples
        ...
        if not val_targets:
             print(f"Warning: No valid validation samples processed in Epoch {epoch+1}. Cannot calculate validation metrics.")
        ...
        print(f"Epoch {epoch+1}/{epochs} - Train Loss: {epoch_loss:.4f}, Train Acc: {epoch_acc:.4f} | Val Loss: {val_loss_epoch:.4f}, Val Acc: {val_acc:.4f}")
    ...
    try:
        ...
        print(f"Loaded best model from {best_model_path}")
         ...
         print(f"Warning: Best model checkpoint {best_model_path} not found. Returning the last state model.")
    except Exception as e:
         print(f"Warning: Error loading best model checkpoint {best_model_path}: {e}. Returning the last state model.")

    return model

# --- Collate Function --- Doesn't appear to be used ---

# Define a collate function to handle None values from dataset errors
...
def classification_main():
    ...
    elif hasattr(torch, 'backends') and hasattr(torch.backends, 'mps') and torch.backends.mps.is_available(): device = torch.device('mps')
    ...
    try:
        ...
        if 'file_path' not in label_df.columns or 'refined_label' not in label_df.columns:
            raise ValueError(f"CSV file {refined_labels_file} must contain 'file_path' and 'refined_label' columns.")
        ...
        # Create a dictionary mapping file path to refined label index
        ...
        if actual_num_classes != num_classes:
            print(f"Warning: Configured num_classes ({num_classes}) does not match the number of unique labels found in {refined_labels_file} ({actual_num_classes}). Using {actual_num_classes} classes found in the label file.")
         ...
         print("Please create this file (e.g., a CSV with 'file_path' and 'refined_label' columns) based on your clustering analysis.")
         return
    ...
    def get_filtered_split(paths):
        ...
        for p_str in paths: # Keep paths as strings as loaded from pickle for now
            ...
            if label is not None:
                ...
                missing_count += 1
        if missing_count > 0:
            print(f"Warning: Could not find labels for {missing_count} images in this split. They will be excluded.")
    ...
    if not train_paths_filt or not val_paths_filt:
        print("Error: No training or validation samples remaining after label mapping. Cannot proceed.")
        return

    print(f"Using {len(train_paths_filt)} train, {len(val_paths_filt)} val, {len(test_paths_filt)} test samples with labels.")

    # Define data transforms (can add augmentation for training)
    ...
    # Create datasets
    try:
        train_dataset = ClassificationDataset(train_paths_filt, train_labels_filt, transform=train_transform)
        val_dataset = ClassificationDataset(val_paths_filt, val_labels_filt, transform=eval_transform)
        test_dataset = ClassificationDataset(test_paths_filt, test_labels_filt, transform=eval_transform) if test_paths_filt else None
    except ValueError as e:
         print(f"Error creating dataset: {e}")
         return

    # Create dataloaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True if device != torch.device('cpu') else False, collate_fn=collate_fn, drop_last=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True if device != torch.device('cpu') else False, collate_fn=collate_fn)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True if device != torch.device('cpu') else False, collate_fn=collate_fn) if test_dataset else None
    ...
    optimizer = optim.AdamW(parameters_to_optimize, lr=learning_rate, weight_decay=1e-4) # Use AdamW

    # Train the model
    print(f"Starting classifier training for {num_classes} classes...")
    trained_model = train_classifier(
        model, train_loader, val_loader, criterion, optimizer, device, epochs, classification_checkpoint_dir, patience=early_stopping_patience
    )
