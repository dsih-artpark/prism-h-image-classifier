...
# Suppress fastdup version check warning
warnings.filterwarnings("ignore")
os.environ["FASTDUP_DISABLE_VERSION_CHECK"] = "1"
...
class ImagePreprocessor:
    ...
    def run_preprocessing(self):
        ...
        fd = fastdup.create(
            input_dir=str(self.input_dir), work_dir=str(self.output_dir / "fastdup_work")
        )
        ...
        broken_filenames = [
            self._extract_filename(path) for path in broken_images_df["filename"].tolist()
        ]
        ...
        clusters_df = self._get_clusters(
            connected_components_df, sort_by="count", min_count=2, ascending=False
        )
        ...
        outlier_filenames = [
            ...
            for path in outlier_df[
                outlier_df.distance < self.outlier_distance
        ...
        blurry_filenames = [
            self._extract_filename(path) for path in blurry_images["filename"].tolist()
        ]
        ...
        problematic_count = {"invalid": 0, "duplicates": 0, "outliers": 0, "dark": 0, "blurry": 0}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess images using fastdup to find and separate invalid, duplicate, outlier, dark, and blurry images."
    )
