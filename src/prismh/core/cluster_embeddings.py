...
def cluster_main():
    ...
    try:
        ...
        print(f"Error: Missing key {e} in {embedding_file}. Ensure it contains 'embeddings' and 'file_paths'.")
        return
    ...
    if embeddings.shape[0] != len(file_paths):
         print(f"Warning: Embeddings count ({embeddings.shape[0]}) != file paths count ({len(file_paths)}). Check {embedding_file}. Alignment issues may occur.")
         # Decide if you want to proceed or exit
         # return

    # Check if the specified image directory exists using Path.is_dir()
    if not image_directory.is_dir():
        print(f"Warning: Image directory '{image_directory}' not found. Fastdup galleries might not show images correctly.")
        # Set input_dir to None if it doesn't exist to avoid fastdup errors trying to read it
    ...
    try:
        ...
        print(f"Fastdup analysis complete. Results stored in {fastdup_work_dir}")
         ...
         print("This might be due to incorrect data types passed to fastdup (e.g., embeddings or filenames).")
    ...
    try:
        # Use string for output_dir
        fd.vis.duplicates_gallery(output_dir=str(fastdup_work_dir), filename="duplicates_gallery.html")
