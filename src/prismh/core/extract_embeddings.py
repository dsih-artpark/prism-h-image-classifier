...
def extract_embeddings_main():
    ...
    if best_model_path.exists():
        ...
        print(
            f"Error: No suitable checkpoint (.pt file starting with 'checkpoint_epoch_' or 'best_model.pt') found in {checkpoint_dir}"
        )
    ...
    def run_extraction(paths, output_filename):
        ...
        if embeddings_list:
            ...
            np.savez_compressed(
                ...
                file_paths=np.array(filepaths_list, dtype="str"),
            )
            print(f"Saved {len(embeddings_np)} embeddings to {output_path}")
        else:
            print(
                f"No embeddings extracted for {output_filename}. This might happen if all images in the split failed to load."
            )
